...
#  modelo 5 (estima todo diferente para ambos grupos, 
#               salvo medias y CvDer [covarianza errores dinámicos])
...
RICLPM5.fit <- lavaan (RICLPM5, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM5.sum <- summary(RICLPM5.fit, standardized = T, fit=TRUE)

RICLPM5.pars <- RICLPM5.sum$PE
RICLPM5.pars <- RICLPM5.pars[!is.na(RICLPM5.pars$z),]
RICLPM5.pars <- RICLPM5.pars[!duplicated(round(RICLPM5.pars$est,6)),]



params5 <- cbind(RICLPM5.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM5.pars[c("est", "se", "pvalue")],3) )
fitmeas5 <- fitmeasures(RICLPM5.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
