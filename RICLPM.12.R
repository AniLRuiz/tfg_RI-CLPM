...
#  modelo 12 (estima todo diferente para ambos grupos, 
#               salvo medias y error dinámico Y [calidad del sueño])
...
RICLPM12.fit <- lavaan (RICLPM12, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM12.sum <- summary(RICLPM12.fit, standardized = T, fit=TRUE)

RICLPM12.pars <- RICLPM12.sum$PE
RICLPM12.pars <- RICLPM12.pars[!is.na(RICLPM12.pars$z),]
RICLPM12.pars <- RICLPM12.pars[!duplicated(round(RICLPM12.pars$est,6)),]



params12 <- cbind(RICLPM12.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM12.pars[c("est", "se", "pvalue")],3) )
fitmeas12 <- fitmeasures(RICLPM12.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
