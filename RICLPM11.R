...
#  modelo 11 (estima todo diferente para ambos grupos, 
#               salvo medias y covarianza ambos residuos)
...
RICLPM11.fit <- lavaan (RICLPM11, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM11.sum <- summary(RICLPM11.fit, standardized = T, fit=TRUE)

RICLPM11.pars <- RICLPM11.sum$PE
RICLPM11.pars <- RICLPM11.pars[!is.na(RICLPM11.pars$z),]
RICLPM11.pars <- RICLPM11.pars[!duplicated(round(RICLPM11.pars$est,6)),]



params11 <- cbind(RICLPM11.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM11.pars[c("est", "se", "pvalue")],3) )
fitmeas11 <- fitmeasures(RICLPM11.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
