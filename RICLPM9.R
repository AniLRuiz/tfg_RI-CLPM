...
#  modelo 9 (estima todo diferente para ambos grupos, 
#               salvo medias y residuo tiempo 1 [calidad del sueño])
...
RICLPM9.fit <- lavaan (RICLPM9, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM9.sum <- summary(RICLPM9.fit, standardized = T, fit=TRUE)

RICLPM9.pars <- RICLPM9.sum$PE
RICLPM9.pars <- RICLPM9.pars[!is.na(RICLPM9.pars$z),]
RICLPM9.pars <- RICLPM9.pars[!duplicated(round(RICLPM9.pars$est,6)),]



params9 <- cbind(RICLPM9.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM9.pars[c("est", "se", "pvalue")],3) )
fitmeas9 <- fitmeasures(RICLPM9.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
