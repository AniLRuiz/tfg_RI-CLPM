...
#  modelo 10 (estima todo diferente para ambos grupos, 
#               salvo medias y varianza residual tiempo 1 x [afecto negativo] )
...
RICLPM10.fit <- lavaan (RICLPM10, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM10.sum <- summary(RICLPM10.fit, standardized = T, fit=TRUE)

RICLPM10.pars <- RICLPM10.sum$PE
RICLPM10.pars <- RICLPM10.pars[!is.na(RICLPM10.pars$z),]
RICLPM10.pars <- RICLPM10.pars[!duplicated(round(RICLPM10.pars$est,6)),]



params10 <- cbind(RICLPM10.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM10.pars[c("est", "se", "pvalue")],3) )
fitmeas10 <- fitmeasures(RICLPM10.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
