...
#  modelo 3 (estima todo diferente para ambos grupos, 
#               salvo g_y)
...
RICLPM3.fit <- lavaan (RICLPM3, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM3.sum <- summary(RICLPM3.fit, standardized = T, fit=TRUE)

RICLPM3.pars <- RICLPM3.sum$PE
RICLPM3.pars <- RICLPM3.pars[!is.na(RICLPM3.pars$z),]
RICLPM3.pars <- RICLPM3.pars[!duplicated(round(RICLPM3.pars$est,6)),]

RICLPM3.sum

params3 <- cbind(RICLPM3.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM3.pars[c("est", "se", "pvalue")],3) )
fitmeas3 <- fitmeasures(RICLPM3.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
