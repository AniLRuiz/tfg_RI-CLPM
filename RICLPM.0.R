...
RICLPM0.fit <- lavaan (RICLPM0, data= academic.tfg, group = "Exam", missing = 'ML',
                          meanstructure = T, int.ov.free = T)

RICLPM0.sum <- summary(RICLPM0.fit, standardized = T, fit=TRUE)

RICLPM0.pars <- RICLPM0.sum$PE
RICLPM0.pars <- RICLPM0.pars[!is.na(RICLPM0.pars$z),]
RICLPM0.pars <- RICLPM0.pars[!duplicated(round(RICLPM0.pars$est,6)),]


params0 <- cbind(RICLPM0.pars[c("lhs", "op", "rhs", "label","group")],
                round(RICLPM0.pars[c("est", "se", "pvalue")],3) )
fitmeas0 <- fitmeasures(RICLPM0.fit, fit.measures = c("npar","logl","aic",
                                                      "rmsea", "cfi", "srmr"))
