...
#  modelo 4 (estima todo diferente para ambos grupos, 
#               salvo medias y g_x - cruzado -> afecto negativo a calidad del sueño)
...
RIy ~~ c(RIyV1, RIyV2)*RIy

RIx ~~ c(CvRIxRIy1, CvRIxRIy2)*RIy

# EEstimar la varianza del tiempo 1 en las variables intrasujeto (latentes) 
...
RICLPM4.fit <- lavaan (RICLPM4, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM4.sum <- summary(RICLPM4.fit, standardized = T, fit=TRUE)

RICLPM4.pars <- RICLPM4.sum$PE
RICLPM4.pars <- RICLPM4.pars[!is.na(RICLPM4.pars$z),]
RICLPM4.pars <- RICLPM4.pars[!duplicated(round(RICLPM4.pars$est,6)),]


params4 <- cbind(RICLPM4.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM4.pars[c("est", "se", "pvalue")],3) )
fitmeas4 <- fitmeasures(RICLPM4.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
