...
#  modelo 8 (estima todo diferente para ambos grupos, 
#               salvo medias y covarianza entre RIy, RIx)
...
RICLPM8.fit <- lavaan (RICLPM8, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM8.sum <- summary(RICLPM8.fit, standardized = T, fit=TRUE)

RICLPM8.pars <- RICLPM8.sum$PE
RICLPM8.pars <- RICLPM8.pars[!is.na(RICLPM8.pars$z),]
RICLPM8.pars <- RICLPM8.pars[!duplicated(round(RICLPM8.pars$est,6)),]



params8 <- cbind(RICLPM8.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM8.pars[c("est", "se", "pvalue")],3) )
fitmeas8 <- fitmeasures(RICLPM8.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
