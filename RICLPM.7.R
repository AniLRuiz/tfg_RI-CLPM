...
#  modelo 7 (estima todo diferente para ambos grupos, 
#               salvo medias y varianza de RIx [afecto negativo])
...
RICLPM7.fit <- lavaan (RICLPM7, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM7.sum <- summary(RICLPM7.fit, standardized = T, fit=TRUE)

RICLPM7.pars <- RICLPM7.sum$PE
RICLPM7.pars <- RICLPM7.pars[!is.na(RICLPM7.pars$z),]
RICLPM7.pars <- RICLPM7.pars[!duplicated(round(RICLPM7.pars$est,6)),]



params7 <- cbind(RICLPM7.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM7.pars[c("est", "se", "pvalue")],3) )
fitmeas7 <- fitmeasures(RICLPM7.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
