...
#  modelo 13 (estima todo diferente para ambos grupos, 
#               salvo medias y errores dinámicos x [afecto negativo])
...
RICLPM13.fit <- lavaan (RICLPM13, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM13.sum <- summary(RICLPM13.fit, standardized = T, fit=TRUE)

RICLPM13.pars <- RICLPM13.sum$PE
RICLPM13.pars <- RICLPM13.pars[!is.na(RICLPM13.pars$z),]
RICLPM13.pars <- RICLPM13.pars[!duplicated(round(RICLPM13.pars$est,6)),]


params13 <- cbind(RICLPM13.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM13.pars[c("est", "se", "pvalue")],3) )
fitmeas13 <- fitmeasures(RICLPM13.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
