...
RICLPM1.fit <- lavaan (RICLPM1, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM1.sum <- summary(RICLPM1.fit, standardized = T, fit=TRUE)

RICLPM1.pars <- RICLPM1.sum$PE
RICLPM1.pars <- RICLPM1.pars[!is.na(RICLPM1.pars$z),]
RICLPM1.pars <- RICLPM1.pars[!duplicated(round(RICLPM1.pars$est,6)),]



params1 <- cbind(RICLPM1.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM1.pars[c("est", "se", "pvalue")],3) )
fitmeas1 <- fitmeasures(RICLPM1.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
