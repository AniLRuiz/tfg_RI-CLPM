#Análisis multigrupo 16 MR (días impares)
#  modelo nulo (estima todo diferente para ambos grupos, 
#               salvo medias y b_x)
...
# stimar la varianza del tiempo 1 en las variables intrasujeto (latentes) 
...
RICLPM2.fit <- lavaan (RICLPM2, data= academic.tfg, group = "Exam", missing = 'ML',
                       meanstructure = T, int.ov.free = T)

RICLPM2.sum <- summary(RICLPM2.fit, standardized = T, fit=TRUE)

RICLPM2.pars <- RICLPM2.sum$PE
RICLPM2.pars <- RICLPM2.pars[!is.na(RICLPM2.pars$z),]
RICLPM2.pars <- RICLPM2.pars[!duplicated(round(RICLPM2.pars$est,6)),]

RICLPM2.sum

params2 <- cbind(RICLPM2.pars[c("lhs", "op", "rhs", "label","group")],
                 round(RICLPM2.pars[c("est", "se", "pvalue")],3) )
fitmeas2 <- fitmeasures(RICLPM2.fit, fit.measures = c("npar","logl","aic",
                                                      "chisq", "df", "pvalue",
                                                      "rmsea", "cfi", "srmr"))
